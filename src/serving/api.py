    ...
    try:
        query = f"""
            WITH target_stations AS (
                ...
                FROM read_parquet('s3://{BUCKET}/gold/alerts/current_status/*.parquet')
            ...
            historical_trends AS (
                ...
                FROM read_parquet('s3://{BUCKET}/silver/velib_stats/*/*.parquet')
        ...
        query = f"""
            WITH reference_data AS (
                ...
                FROM (
                    ...
                    FROM read_json_auto('s3://{BUCKET}/bronze/velib/reference/station_information.json')
                )
            ),
            silver_data AS (
